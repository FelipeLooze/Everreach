...
def delete_campaign(db: Session, campaign_id: str) -> bool:
    ...
    # Phase 15K — LocationEconomy/LocalSupplyLevel (Phase 14) are
    # generated by every seeded campaign now too, same class of
    # pre-existing gap as Organization above.
    db.query(LocalSupplyLevel).filter(LocalSupplyLevel.location_id.in_(location_ids)).delete(
        synchronize_session=False
    )
    db.query(LocationEconomy).filter(LocationEconomy.location_id.in_(location_ids)).delete(
        synchronize_session=False
    )
