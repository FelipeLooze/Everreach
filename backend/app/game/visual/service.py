...
copy failure — is caught and turned into a FAILED VisualGenerationRequest
row, never a raised exception. The exception is caller programming
errors (an unregistered workflow_key/version) — those are raised
immediately, before any request row is even created, since no attempt
was actually made.
"""
...
def _classify_comfyui_error(exc: ComfyUIClientError) -> str:
    ...
    if "disabled" in message or "could not reach" in message:
        return "COMFYUI_OFFLINE"
    if "took too long" in message:
        return "GENERATION_TIMEOUT"
    if "rejected" in message:
        return "COMFYUI_REJECTED_WORKFLOW"
...
def request_visual_asset(
    ...
    *,
    ...
    if not comfyui_client.is_available():
        return _fail(db, request.id, "COMFYUI_OFFLINE", "ComfyUI is disabled or unreachable.")
    ...
    try:
        ...
        return _fail(db, request.id, "WORKFLOW_NOT_FOUND", str(exc))
    ...
    try:
        ...
        return _fail(db, request.id, "UNKNOWN_ERROR", str(exc))
    ...
    if image_ref is None:
        return _fail(db, request.id, "OUTPUT_NOT_FOUND", "ComfyUI history had no output image.")

    try:
        raw_path = comfyui_client.resolve_output_path(image_ref["subfolder"], image_ref["filename"])
    except ComfyUIClientError as exc:
        return _fail(db, request.id, "OUTPUT_NOT_FOUND", str(exc))
    ...
    try:
        ...
        return _fail(db, request.id, "FILE_COPY_FAILED", str(exc))
    ...
    try:
        ...
        return _fail(db, request.id, "INVALID_OUTPUT", f"Could not read generated image: {exc}")
