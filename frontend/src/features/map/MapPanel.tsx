...
export function MapPanel({ campaignId }: { campaignId: string }) {
    ...
    <div>
      {map.regions.map((region) => (
        <div key={region.id} className="map-region">
          ...
          <p className="map-boss">Chefe Principal da Região: {region.main_boss_name === "UNKNOWN" ? "DESCONHECIDO" : region.main_boss_name}</p>
          <ul>
            {map.locations
